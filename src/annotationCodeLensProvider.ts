  ...
  provideCodeLenses(
    ...
    for (const [, annotations] of this.annotations) {
      const matches = annotations.filter(a => {
        const normalizedAnnotationPath = a.filePath.replace(/\\/g, '/');
        // Match if the file path ends with the annotation's relative path
        // Ensure we match at a path boundary (preceded by / or start of string)
        return normalizedFilePath.endsWith(normalizedAnnotationPath) &&
          (normalizedFilePath === normalizedAnnotationPath ||
           normalizedFilePath.endsWith('/' + normalizedAnnotationPath));
      });
      fileAnnotations = fileAnnotations.concat(matches);
    }
